...
export default function AddLocalUser({ onUserAdd = () => {} }) {
  ...
  const [userCredentials, setUserCredentials] = useState({ username: '', password: '', role: '' });
      ...
      >
        <>
          ...
          <form onSubmit={handleSubmit} className="w-100 text-sm mt-4">
            ...
            <fieldset className="w-100 flex flex-col mb-7">
              <label htmlFor="role" className="w-100 text-slate-500 font-medium mb-1">Role</label>
              <input id="role" type="text" name="role" required className="w-100 border-2 border-slate-200 rounded-lg p-2" placeholder="Enter user's role" onChange={handleInput} />
            </fieldset>
